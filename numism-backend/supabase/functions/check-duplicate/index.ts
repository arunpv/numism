...
// mint_year+mint_mark (nulls match nulls) — a heuristic flag, not a DB constraint.
...
export default {
  fetch: withSupabase({ auth: ["publishable"] }, async (req, ctx) => {
    ...
    let query = ctx.supabaseAdmin
      .from("personal_coins")
      .select("id, image_path, image_quality_score, personal_notes, quantity, album_id, page_number, pocket_number")
      .eq("country", country)
