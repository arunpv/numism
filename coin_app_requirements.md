...
*   **Frontend:** Hosted at `www.orchestrun.com/numism`, as a new route (`src/app/numism/`) inside the existing **Orchestrun-Web** Next.js 16 App Router repo — not a separate app/repo/deploy. Orchestrun is on GoDaddy hosting with a GitHub-connected deploy and no reverse-proxy layer, so a same-repo route is the only path that doesn't require new infra; it rides the existing deploy pipeline. Code stays logically isolated under `src/app/numism/` (own components, own Supabase client init) — it does not touch the marketing pages, and shares only the repo/build/deploy, not app logic. Installable as a PWA; uses the browser `MediaDevices`/`<input capture>` camera APIs — works on both desktop and mobile browsers, no separate native codebase.
