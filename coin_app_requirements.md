...
*   **User-maintained reference data, not AI-guessed.** Each country has a fixed, known set of mints, so the user pre-populates the reference data themselves from the Mints tab — before scanning a batch of coins from that country. No AI call is involved in deciding what a mark *means*; extraction only reads the raw `mint_mark` (and, when relevant, its position — see below).
*   **A (country, mark) pair doesn't always mean one mint** — updated 2026-09-17 after finding real cases where it doesn't: India's `★` means Hyderabad plain, but Seoul or Taegu (Korean mints that struck coins for India) depending on whether it sits below the first or last digit of the year; India's `♦` means Mumbai plain, or the Royal Mint, London when positioned the same way; the US `D` means Denver, except 1838–1861 when it means Dahlonega. A single flat `mints` table couldn't express this, so it's now two tables: `mints` (pure identity — country, mint_name, optional reference photo) and **`mint_mark_rules`** (country, mint_mark, and one of: `default` — the plain/fallback meaning; `year_range` — applies only when `mint_year` falls in `[year_min, year_max]`; `position` — applies only when the mark sits `first_digit`/`last_digit` relative to the year — each pointing at a `mints` row). Most marks only ever have a `default` rule, so this is a strict superset of the old one-mark-one-mint case, not a burden on the common case.
*   **Resolution flow:** once `country` + `mint_mark` (+ `mint_year`, + `mint_mark_position` when applicable) are known (from AI extraction or user edit) during the review step (§3.3), the client resolves against `mint_mark_rules` — matched case-insensitively/trimmed — preferring a `position` match, then a `year_range` match, then falling back to `default`. A blank `mint_mark` resolves against the literal sentinel `"No Mark"` (some countries have a designated "no mark visible" mint, e.g. Philadelphia/Kolkata). If nothing resolves, the review form shows "no mint on file for {country} / {mint_mark}" rather than blocking save — the coin still saves with its raw `mint_mark`/`mint_mark_position`; the user can add the missing rule afterward and it'll resolve for that coin and all future ones automatically.
*   `personal_coins.mint_id` stores the resolved FK (nullable), and `mint_mark_position` stores the extracted/edited position alongside the existing `mint_mark` (both nullable, for record/audit and so re-resolution on edit stays consistent) — resolution is a join, not a copied string, so fixing a mint name or rule in one place fixes it everywhere it's used.
*   Resolution also runs server-side in `save-coin`/`update-coin` at write time (not just client-side preview) so the stored `mint_id` is always consistent with the reference data at the moment of saving, and doesn't depend on the client having done the lookup correctly.
*   **Constrained extraction, once marks are on file.** Free-text AI extraction of a mint mark is unreliable (marks are often tiny/worn, and even a correct read may not exactly match the wording stored in the reference data). Once there are known marks on file for the AI-guessed `country`, `extract-coin` (§5.2) makes a **second Gemini call**, re-examining the same image but constrained to only the known marks for that country (`responseSchema` enum) plus `null` — turning "guess what symbol this is" into "pick which of these known marks this matches, or say none." The same call also asks for `mint_mark_position` (nullable `first_digit`/`last_digit` enum), left null unless the mark's exact placement relative to a year digit is what would distinguish the mint — true for only a few marks so far (India's `★`/`♦`), irrelevant for the rest. This second pass is skipped (no extra API call) when no marks are on file yet for that country. `extract-coin` now also returns `mint_id`/`mint_name` directly, resolved server-side, so the review step doesn't need a separate round trip for the common case (only calling `resolve-mint` again if the user manually edits the mark, year, or position).
...
*   **Manual override:** the Queue tab also has a "Process now" button (`src/lib/processBatch.ts`) that runs the same extraction step directly on the main thread in response to an explicit click, skipping the WiFi/screen-off gates entirely — a foreground click is inherently neither of those, and it's the user deliberately overriding the automatic timing. Kept out of `batchQueue.ts`/`sw.ts` since it depends on `coinApi` (the Supabase client), which isn't safe to load in the service worker's scope. Retrying an errored item still goes through the gated automatic path (resets it to `pending` and re-registers the sync task).
*   **Stuck-`processing` recovery — added 2026-09-19.** In practice, a large batch (~20 coins) mostly stalled: Background Sync doesn't guarantee a service worker finishes a long loop before the browser/OS kills it, and on Android it's also mostly triggered by a connectivity *change*, not by continuously being on WiFi — so once the phone stayed connected, nothing re-fired further attempts at all. Every entry point (`processQueueNow`, `sw.ts`'s `processBatch`) now calls `batchQueue.reclaimStuckProcessing()` first, which resets any item still sitting in `processing` (orphaned by an interrupted prior run) back to `pending` so the next run — automatic or manual — actually picks it up instead of leaving it stuck forever. Given the connectivity-change reliability gap, the manual "Process now" button is the more dependable way to work through a large backlog day-to-day, not just a fallback.
